...
class DCEC(object):
    ...
    def pretrain(self, x, batch_size=256, epochs=200, optimizer='adam', save_dir='results/temp'):
        print('...Pretraining...')
        self.cae.compile(optimizer=optimizer, loss='mse', metrics=['accuracy'])
        from keras.callbacks import CSVLogger
        csv_logger = CSVLogger(args.save_dir + '/pretrain_log.csv')

        # andree set this epoch:
        #epochs = 50
...
if __name__ == "__main__":
    ...
    # load dataset
    from datasets import load_mnist, load_usps, load_fashion_mnist, load_cifar10
    if args.dataset == 'mnist':
        ...
        x, y = load_cifar10()
        #y = np.array([int(np.squeeze(y[i])) for i in range(len(y))])
